...
class VcsClientClassSourceTest {

    @Test
    void testConstructorValidation() {
        assertThrows(NullPointerException.class, () -> {
            new VcsClientClassSource(null);
        });
    }
    ...
    void testLoadClassData() throws IOException {
        ...
        VcsClientClassSource source = new VcsClientClassSource(client);
    ...
    void testLoadClassDataThrowsIOException() throws IOException {
        ...
        when(client.readFile("com/example/MyClass.class"))
            .thenThrow(new IOException("File not in repository"));
        ...
        assertTrue(thrown.getMessage().contains("File not in repository"));
    }
    ...
    void testCanLoadReturnsFalseOnIOException() throws IOException {
        ...
        when(client.exists("com/example/MyClass.class"))
            .thenThrow(new IOException("Repository error"));
    ...
    void testGetDescription() {
        VcsClient client = mock(VcsClient.class);
        when(client.getDescription()).thenReturn("Git[repo=/opt/app, branch=main]");

        VcsClientClassSource source = new VcsClientClassSource(client);

        assertEquals("VcsClientClassSource[Git[repo=/opt/app, branch=main]]", source.getDescription());
    }
